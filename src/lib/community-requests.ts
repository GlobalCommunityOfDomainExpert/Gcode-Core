...
export function addCommunityRequest(input: {
  eventId: string;
  stakeholderId: string;
  category: StakeholderCategory;
  message: string;
...
export function getCommunityRequestsByEvent(
  ...
  return communityRequests.filter((request) => request.eventId === eventId);
}

export function getCommunityRequestById(
  id: string,
): CommunityRequest | undefined {
  return communityRequests.find((request) => request.id === id);
}
...
export function respondToCommunityRequest(
  ...
  const request = getCommunityRequestById(id);
...
export function confirmCommunityRequest(
  id: string,
): CommunityRequest | undefined {
  const request = getCommunityRequestById(id);
  if (!request) return undefined;
...
export function removeCommunityRequest(id: string): void {
  const index = communityRequests.findIndex((request) => request.id === id);
  if (index !== -1) communityRequests.splice(index, 1);
}

export function nudgeCommunityRequest(
  id: string,
): CommunityRequest | undefined {
  const request = getCommunityRequestById(id);
  if (!request) return undefined;
  request.remindedAt = new Date().toISOString();
  return request;
}
