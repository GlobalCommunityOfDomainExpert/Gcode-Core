...
async function attempt(n) {
  ...
  page.on('response', async (res) => {
    if (res.url().includes('razorpay-order')) {
      try { orderBody = await res.text(); } catch {}
    }
  });
  await page.goto('http://localhost:3000/events/21/register', { waitUntil: 'networkidle' });
  ...
  const failed = text.includes('Registration failed') || text.includes('returned no data');
